...
const UserProfile = () => {
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			...
			if (token) {
				try {
					const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/profiles/user`, {
						headers: {
					...
					console.log(error);
					showErrorSnackbar("Error fetching user data:", error);
				}
			}
		};
		...
		if (token) {
			try {
				...
				if (response.ok) {
					...
					showErrorSnackbar("Failed to update user data");
				}
			} catch (error) {
				showErrorSnackbar("Error updating user data:", error);
			}
		}
	};
